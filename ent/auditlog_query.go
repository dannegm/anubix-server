...
func (_q *AuditLogQuery) QueryUser() *UserQuery {
...
func (_q *AuditLogQuery) QueryDevice() *DeviceQuery {
...
func (_q *AuditLogQuery) QueryEntry() *EntryQuery {
...
func (_q *AuditLogQuery) WithUser(opts ...func(*UserQuery)) *AuditLogQuery {
	query := (&UserClient{config: _q.config}).Query()
...
func (_q *AuditLogQuery) WithDevice(opts ...func(*DeviceQuery)) *AuditLogQuery {
	query := (&DeviceClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *AuditLogQuery) WithEntry(opts ...func(*EntryQuery)) *AuditLogQuery {
	query := (&EntryClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *AuditLogQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*AuditLog, error) {
	...
	if _q.withUser != nil || _q.withDevice != nil || _q.withEntry != nil {
		withFKs = true
	}
	if withFKs {
...
func (_q *AuditLogQuery) loadDevice(ctx context.Context, query *DeviceQuery, nodes []*AuditLog, init func(*AuditLog), assign func(*AuditLog, *Device)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*AuditLog)
	for i := range nodes {
		if nodes[i].device_audit_logs == nil {
			continue
		}
		fk := *nodes[i].device_audit_logs
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "device_audit_logs" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *AuditLogQuery) loadEntry(ctx context.Context, query *EntryQuery, nodes []*AuditLog, init func(*AuditLog), assign func(*AuditLog, *Entry)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*AuditLog)
	for i := range nodes {
		if nodes[i].entry_audit_logs == nil {
			continue
		}
		fk := *nodes[i].entry_audit_logs
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "entry_audit_logs" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
