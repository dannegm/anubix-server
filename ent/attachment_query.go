...
func (_q *AttachmentQuery) QueryEntry() *EntryQuery {
...
func (_q *AttachmentQuery) WithEntry(opts ...func(*EntryQuery)) *AttachmentQuery {
	query := (&EntryClient{config: _q.config}).Query()
...
func (_q *AttachmentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Attachment, error) {
	...
	if query := _q.withEntry; query != nil {
		if err := _q.loadEntry(ctx, query, nodes, nil,
			func(n *Attachment, e *Entry) { n.Edges.Entry = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *AttachmentQuery) loadEntry(ctx context.Context, query *EntryQuery, nodes []*Attachment, init func(*Attachment), assign func(*Attachment, *Entry)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*Attachment)
	for i := range nodes {
		if nodes[i].entry_attachments == nil {
			continue
		}
		fk := *nodes[i].entry_attachments
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "entry_attachments" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
