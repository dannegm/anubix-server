...
func (_q *TagQuery) QueryEntryTags() *EntryTagQuery {
...
func (_q *TagQuery) WithEntryTags(opts ...func(*EntryTagQuery)) *TagQuery {
	query := (&EntryTagClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *TagQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Tag, error) {
	...
	if query := _q.withEntryTags; query != nil {
		if err := _q.loadEntryTags(ctx, query, nodes,
			func(n *Tag) { n.Edges.EntryTags = []*EntryTag{} },
			func(n *Tag, e *EntryTag) { n.Edges.EntryTags = append(n.Edges.EntryTags, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *TagQuery) loadEntryTags(ctx context.Context, query *EntryTagQuery, nodes []*Tag, init func(*Tag), assign func(*Tag, *EntryTag)) error {
	...
	query.Where(predicate.EntryTag(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(tag.EntryTagsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.tag_entry_tags
		if fk == nil {
			return fmt.Errorf(`foreign-key "tag_entry_tags" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "tag_entry_tags" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
