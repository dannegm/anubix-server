...
func (_q *UserQuery) QueryShareTokens() *ShareTokenQuery {
...
func (_q *UserQuery) WithShareTokens(opts ...func(*ShareTokenQuery)) *UserQuery {
	query := (&ShareTokenClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := _q.withShareTokens; query != nil {
		if err := _q.loadShareTokens(ctx, query, nodes,
			func(n *User) { n.Edges.ShareTokens = []*ShareToken{} },
			func(n *User, e *ShareToken) { n.Edges.ShareTokens = append(n.Edges.ShareTokens, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *UserQuery) loadVaults(ctx context.Context, query *VaultQuery, nodes []*User, init func(*User), assign func(*User, *Vault)) error {
	...
	query.Where(predicate.Vault(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.VaultsColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_vaults" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (_q *UserQuery) loadShareTokens(ctx context.Context, query *ShareTokenQuery, nodes []*User, init func(*User), assign func(*User, *ShareToken)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.ShareToken(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.ShareTokensColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.user_share_tokens
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_share_tokens" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_share_tokens" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
