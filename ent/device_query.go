...
func (_q *DeviceQuery) QueryUser() *UserQuery {
...
func (_q *DeviceQuery) QueryAuditLogs() *AuditLogQuery {
...
func (_q *DeviceQuery) WithUser(opts ...func(*UserQuery)) *DeviceQuery {
	query := (&UserClient{config: _q.config}).Query()
...
func (_q *DeviceQuery) WithAuditLogs(opts ...func(*AuditLogQuery)) *DeviceQuery {
	query := (&AuditLogClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *DeviceQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Device, error) {
	...
	if query := _q.withAuditLogs; query != nil {
		if err := _q.loadAuditLogs(ctx, query, nodes,
			func(n *Device) { n.Edges.AuditLogs = []*AuditLog{} },
			func(n *Device, e *AuditLog) { n.Edges.AuditLogs = append(n.Edges.AuditLogs, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *DeviceQuery) loadUser(ctx context.Context, query *UserQuery, nodes []*Device, init func(*Device), assign func(*Device, *User)) error {
...
func (_q *DeviceQuery) loadAuditLogs(ctx context.Context, query *AuditLogQuery, nodes []*Device, init func(*Device), assign func(*Device, *AuditLog)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*Device)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.AuditLog(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(device.AuditLogsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.device_audit_logs
		if fk == nil {
			return fmt.Errorf(`foreign-key "device_audit_logs" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "device_audit_logs" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
