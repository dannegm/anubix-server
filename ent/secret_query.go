...
func (_q *SecretQuery) QueryBlock() *BlockQuery {
...
func (_q *SecretQuery) WithBlock(opts ...func(*BlockQuery)) *SecretQuery {
	query := (&BlockClient{config: _q.config}).Query()
...
//
//	client.Secret.Query().
//		GroupBy(secret.FieldCiphertext).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
func (_q *SecretQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Secret, error) {
	...
	if query := _q.withBlock; query != nil {
		if err := _q.loadBlock(ctx, query, nodes, nil,
			func(n *Secret, e *Block) { n.Edges.Block = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *SecretQuery) loadBlock(ctx context.Context, query *BlockQuery, nodes []*Secret, init func(*Secret), assign func(*Secret, *Block)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*Secret)
	for i := range nodes {
		if nodes[i].block_secrets == nil {
			continue
		}
		fk := *nodes[i].block_secrets
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "block_secrets" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
